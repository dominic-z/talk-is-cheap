...
public class AdvancedEtcdServiceImpl implements AdvancedEtcdService {
    ...
    public boolean cas(String key, String expectValue, String updateValue) throws Exception {
        ...
        log.info("key: {}, expectValue:{}, updateValue:{}", key, expectValue, updateValue);
        ...
        TxnResponse txnResponse = getClient().getKVClient()
                ...
                .commit()
                .get();
        log.info("txnResponse: {}", txnResponse);

        log.info("txnResponse.isSucceeded(): {}, txnResponse.getPutResponses(): {}", txnResponse.isSucceeded(),
                ...
                .thenAccept(result -> {
                    ...
                    kvClient.put(bytesOf(key), bytesOf(value), putOption)
                            .thenAccept(putResponse -> {
                                log.info("put操作完成，putResponse:{}，开始设置续约操作",putResponse);
                                // put操作完成后，再设置无限续租的操作
