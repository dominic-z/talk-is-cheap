...
SELECT * FROM task_execution te WHERE task_startup_id =99 order by create_time desc;
...
SELECT sd.name,sd.id ,se.* FROM stage_execution se join stage_startup ss join stage_definition sd  on  ss.task_execution_id=82 and ss.id=se.stage_startup_id and ss.stage_id =sd.id  order by se.create_time desc;
