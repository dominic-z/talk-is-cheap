...
SELECT * FROM task_execution te WHERE task_startup_id =149 order by create_time desc;
SELECT sd.name ,ss.* FROM stage_startup ss join task_execution te join stage_definition sd on te.task_startup_id=190 and te.id=ss.task_execution_id and  ss.stage_id = sd.id  order by ss.create_time desc;
...
(SELECT id FROM task_execution te WHERE task_startup_id =182 order by create_time desc limit 1) and ss.id=se.stage_startup_id and ss.stage_id =sd.id  order by se.create_time desc;
