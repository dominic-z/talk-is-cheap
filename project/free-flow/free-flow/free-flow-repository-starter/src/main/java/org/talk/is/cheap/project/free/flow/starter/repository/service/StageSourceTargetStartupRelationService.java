...
public class StageSourceTargetStartupRelationService{
    ...
    // 深度分页的service接口
    public List<StageSourceTargetStartupRelation> selectByExampleDeepPaging(StageSourceTargetStartupRelationExample example) {
        if (example == null) {
            return new ArrayList<>();
        }
        ...
        return stageSourceTargetStartupRelationMapper.selectByExampleDeepPagingByIdSubQuery(example);
    }
    ...
    @Transactional(rollbackFor = Exception.class, transactionManager = "repositoryStarterTransactionManager")
    public int createBatchSelective(Collection<StageSourceTargetStartupRelation> records, Collection<String> excludeColNames) {
        if (records == null || records.isEmpty()) {
            return 0;
        }
        return stageSourceTargetStartupRelationMapper.insertBatchSelective(records, new HashSet<String>(excludeColNames));
    }
