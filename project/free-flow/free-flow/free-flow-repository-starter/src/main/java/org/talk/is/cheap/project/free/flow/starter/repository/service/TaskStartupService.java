...
public class TaskStartupService{
    ...
    @Transactional(rollbackFor = Exception.class, transactionManager = "repositoryStarterTransactionManager")
    public int createBatchSelective(Collection<TaskStartup> records, Collection<String> excludeColNames) {
        if (records == null || records.isEmpty()) {
            return 0;
        }
        return taskStartupMapper.insertBatchSelective(records, new HashSet<String>(excludeColNames));
    }
    // 基于taskStartupDao
