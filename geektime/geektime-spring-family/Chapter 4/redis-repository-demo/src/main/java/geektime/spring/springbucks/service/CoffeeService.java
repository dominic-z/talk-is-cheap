...
public class CoffeeService {
    // 下面这两个repository是根据model类的注解来判断不同类型的数据源的repository
    @Autowired
    ...
    public Optional<Coffee> findSimpleCoffeeFromCache(long id) {
        Optional<CoffeeCache> cached = cacheRepository.findById(id);
        if (cached.isPresent()) {
            CoffeeCache coffeeCache = cached.get();
            Coffee coffee = Coffee.builder()
                    .name(coffeeCache.getName())
            ...
            Optional<Coffee> raw = coffeeRepository.findById(id);
            raw.ifPresent(c -> {
                CoffeeCache coffeeCache = CoffeeCache.builder()
                        .id(c.getId())
                        .name(c.getName())
