package dynamicSql.dynamicChoose;
...
public class TestMyBatis {
    ...
    {
        try {
            resource = Resources.getResourceAsReader("dynamicSql/dynamicChoose/mybatis-config.xml");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    void test1(){
        try(SqlSession sqlSession=sqlSessionFactory.openSession()){
            ...
            List<Blog> blogList;
            blogList=blogMapper.selectBlogsByChoose(null,null);
            System.out.println(blogList);

            blogList=blogMapper.selectBlogsByChoose("content11",null);
            System.out.println(blogList);

            Author author=new Author();
            author.setName("li");
            blogList=blogMapper.selectBlogsByChoose(null,author);
            System.out.println(blogList);
