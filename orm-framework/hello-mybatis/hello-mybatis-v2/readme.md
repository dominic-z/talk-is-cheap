...
2. MybatisAutoConfiguration源码：核心就是创建一个SqlSessionFactory和SqlSessionTemplate，对于SqlSessionFactory的创建，默认是通过api的方式设置的，也可以直接读取mybatis-conf.xml配置文件进行设置，如果希望自定义sqlSessionTemplate，直接抄代码就行。另外，注意到SqlSessionFactoryBeanCustomizer这个东西，似乎是给开发者开的自定义factory的口子，没做尝试
